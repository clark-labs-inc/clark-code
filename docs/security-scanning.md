...
flowchart TD
    ...
    Skill --> Policy["Host model policy: ~deepseek/deepseek-v4-flash-latest"]
    ...
    Deep --> Delegates["Bounded read-only ~deepseek/deepseek-v4-flash-latest delegates"]
...
- a host-owned `~deepseek/deepseek-v4-flash-latest` model policy for the whole root scan turn;
...
`delegate_read_only` and `resolve_delegation` on the first DeepSeek V4 Flash Latest model call.
...
3. The provider pins the model to `~deepseek/deepseek-v4-flash-latest` and its
   highest supported `xhigh` reasoning effort, regardless of any conversation
   model setting, and exposes the
...
DeepSeek V4 Flash Latest owns semantic work:
...
harness; this does not change the product workflow's DeepSeek V4 Flash Latest default. The
