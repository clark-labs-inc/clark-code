...
flowchart LR
    ...
    Queue --> Analyzer["z-ai/glm-5.2 analysis workers"]
...
production model route (`z-ai/glm-5.2`), trigger, parent run, target snapshot, and
...
`z-ai/glm-5.2` may return `no_match_in_automated_sources`, but that state is
...
- production semantic phases require exact `z-ai/glm-5.2`;
