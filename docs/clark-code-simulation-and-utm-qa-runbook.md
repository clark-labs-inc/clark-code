...
Ten deterministic macOS product-profile contract tests pass, including a real
custom-origin WebKit persistence test for nonsecret settings and a native
encrypted credential-envelope round trip. The authenticated non-model macOS
`auth-smoke` passed on 2026-08-04 under
`target/macos-product-journey/20260804-runtime-registry-proof-11/`. It proved a
real existing remote conversation open, same-process warm reattach with no
additional connection, graceful shutdown, full process restart and reopen,
one native account worker per process, Linux remote residency, encrypted native
auth retention, same-account disposable key creation/revocation, disposable
conversation deletion, profile erasure, unchanged personal state, and
restoration of the prior app process state. The journey uses the laptop's real
configured `nucleus` SSH entry read-only; it does not copy a private key or
create an alternate SSH configuration. The existing personal product profile
remains a protected read-only boundary.
...
```text
<repo>/target/debug/bundle/macos/Clark Code Dev.app
```
...
- `seed`: validate an owner-only bootstrap and write account-scoped non-secret
  project/model settings plus a random run marker;
...
The transient WebKit bootstrap contains no credential. The helper accepts it
...
- model exactly `clark-code:free`;
- lowercase account scope beginning with `id:`;
...
Authentication is written separately as the exact app-owned v2 encrypted
native credential envelope under the isolated app-data directory. The helper
asserts that credential-shaped fields and the unscoped settings key are absent.
No provider key or ownership record is placed in WebView storage;
the real product provisions its disposable desktop key through native account
authority.

#### 12.7.7 Product-state probe

The product is launched and observed, fully stopped, then relaunched and
observed again before the helper reopens the custom store. The probe validates:

- project path equals the disposable workspace;
- model equals the included coding route;
- project/model settings use the exact account-scoped keys;
- unscoped settings, WebView auth, provider key, and provider-key owner are absent;
- random run marker is unchanged.

The separate native probe decrypts the isolated envelope and verifies that the
expected authentication survived restart, the file is encrypted, and token,
email, and account values do not occur in plaintext. Neither probe emits a
token, key, raw account ID, email address, project content, or storage payload.
...
2. writes the exact encrypted native credential envelope into the guest's
   app-private data directory;
3. opens the WebKit database and decodes nonsecret settings as UTF-16LE;
4. clears the disposable WebKit storage database;
5. writes only account-scoped project and model settings;
6. commits and checkpoints SQLite;
7. restores ownership to `home`;
8. relaunches in the graphical session;
9. proves the native envelope is retained, encrypted, and absent from WebKit;
10. polls product state until the account, project, and model all match.
...
calls the native `clark_provision_code_key` command. The native host binds the
Clark Code key to the validated account and rewrites the encrypted app-private
credential envelope atomically. The renderer receives no key value and WebKit
stores no provider-key or owner field.

The Ubuntu journey validates that the native encrypted credential files exist,
that the account-scoped project and model settings are correct, and that WebKit
contains no credential-shaped fields. It never returns a key or token.
...
### 20.15 Native credential provisioning failure

**Signal:** the authenticated account is present but the native encrypted
credential envelope is absent or cannot open a session.

**Response:** verify the Clark endpoint and native account authority without
logging tokens, allow a bounded initialization retry, and fail closed. Never
paste credentials into the renderer or WebView storage.

### 20.16 WebView2 policy remains
...
- keep exact project/model assertions in the WebKit probe and account/session
  assertions in the native encrypted credential probe;
...
**Signal:** `profile-bootstrap.json` still exists after seed or cleanup.
...
12. Native encrypted macOS credential bootstrap versus the app-owned credential
    directory.
13. Ad-hoc-signed nonsecret WebKit profile helper versus the stably signed
    product/helper.
...
- seeds and launches the isolated QA store, then stops and relaunches it;
...
Both jobs are dependencies of every desktop and remote-worker build.
