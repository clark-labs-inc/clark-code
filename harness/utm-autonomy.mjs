...
import {
  bundleDataPath,
  ejectRemovableMedia,
  ensureRemovableDrive,
  qemuArgumentStrings,
  readUtmConfig,
  setQemuAdditionalArguments,
  setRemovableDriveSource,
} from "./utm-config.mjs";
...
async function provisionUbuntu(environment, credentials, hostIp) {
  ...
  await stopVm(environment.vm_name, events);
  setRemovableDriveSource(environment.vm_name, 1, isoPath);
  ...
  ensureRemovableDrive(environment.vm_name, 3, seedIso);
  let installed = false;
  let mediaEjected = false;
  let confirmations = 0;
  try {
    ensureQmpArguments(
      environment.vm_name,
    ...
    while (vmStatus(environment.vm_name) !== "stopped" && Date.now() < deadline) {
      ...
      if (allocatedBytes < 8 * 1024 * 1024 && Date.now() >= nextConfirmation) {
    ...
    if (vmStatus(environment.vm_name) !== "stopped") {
      throw new Error("Ubuntu unattended Desktop installation exceeded one hour");
    }
    installed = true;
    events.push({
    ...
    if (!installed && vmStatus(environment.vm_name) === "started") {
      await stopVm(environment.vm_name, events);
    }
  }
  try {
    ...
    ejectRemovableMedia(environment.vm_name, [
      path.basename(isoPath),
      path.basename(seedIso),
    ]);
    mediaEjected = true;
    events.push({
      action: "installer_media_eject",
      status: "passed",
      method: "utm_accessibility_api",
    });
    await stopVm(environment.vm_name, events);
    ensureQmpArguments(environment.vm_name, environment.autonomy.qmp_port);
    await startVm(environment.vm_name, events);
    ...
    if (mediaEjected && existsSync(seedIso)) rmSync(seedIso);
  }
}
