...
const SOURCE_TRANSFER_TIMEOUT_MS = 30 * 60_000;
const SOURCE_TRANSFER_ATTEMPTS = 5;
const SOURCE_TRANSFER_RETRY_DELAY_MS = 3_000;
const GUESTS = {
  windows: {
...
export function pushSourceArchive({
  runner,
  vmName,
  guestArchive,
  archiveBytes,
  attempts = SOURCE_TRANSFER_ATTEMPTS,
  retryDelayMs = SOURCE_TRANSFER_RETRY_DELAY_MS,
  wait = sleep,
}) {
  let lastResult = null;
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    if (attempt > 1) wait(retryDelayMs);
    lastResult = runner(
      "utmctl",
      ["file", "push", vmName, guestArchive],
      {
        input: archiveBytes,
        timeout_ms: SOURCE_TRANSFER_TIMEOUT_MS,
      },
    );
...
export function excludeDeletedSourcePaths(paths, deletedPaths) {
  const deleted = new Set(deletedPaths);
  return paths.filter((relative) => !deleted.has(relative));
}
...
function gitSourcePaths() {
  ...
  const deleted = run("git", ["ls-files", "--deleted", "-z"], { binary_output: true });
  if (!deleted.ok) throw new Error(`cannot enumerate deleted source files: ${deleted.stderr}`);
  ...
  const deletedPaths = deleted.stdout
    ...
    .split("\0")
    .filter(Boolean);
  return validateSourcePaths(excludeDeletedSourcePaths(paths, deletedPaths));
}
...
export function stageSourceToGuest(platform, sourcePackage) {
  ...
  const pushed = pushSourceArchive({
    runner: run,
    vmName: guest.vm_name,
    guestArchive,
    archiveBytes: readFileSync(sourcePackage.archivePath),
  });
  if (!pushed.ok) {
    return {
      platform,
      ...
      transfer_attempts: pushed.attempts,
    };
  }
