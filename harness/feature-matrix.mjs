...
function findBalanced(source, marker, open, close) {
  ...
  for (let index = start; index < source.length; index += 1) {
    ...
    if (char === "'") {
      ...
      for (let cursor = index + 1; cursor < source.length && source[cursor] !== "\n"; cursor += 1) {
        if (quoteEscaped) quoteEscaped = false;
        else if (source[cursor] === "\\") quoteEscaped = true;
...
function verifyBalancedParser() {
  const rust = `trait Provider {
    /// Export the provider's canonical history.
    fn transcript(&self) -> BoxFuture<'static, Result<{ answer: String }>>;
  }`;
  const body = findBalanced(rust, "trait Provider", "{", "}");
  if (!body.includes("fn transcript")) {
    throw new Error("balanced-block parser regression: Rust trait body was truncated");
  }
}

verifyBalancedParser();
...
async function validateContracts() {
  ...
  for (const [inventoryId, spec] of Object.entries(inventory.inventories)) {
    ...
    if (
      ...
      && manifest.live_model.catalog_required !== false
      && manifest.live_model.product_alias
