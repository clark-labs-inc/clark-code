...
test("candidate release is parallel, supply-chain bound, and platform independent", () => {
  ...
  const scientistStagingScript = readFileSync(
    new URL("../scripts/stage-scientist-worker.sh", import.meta.url),
    "utf8",
  );
  const ordinaryStagingScript = readFileSync(
  ...
  const windowsTestManifest = readFileSync(
    new URL("../src-tauri/windows-test.manifest", import.meta.url),
    "utf8",
  );
  const windowsPeCheck = readFileSync(
    new URL("../scripts/check-windows-pe-imports.ps1", import.meta.url),
    "utf8",
  );
  ...
  assert.match(
    workflow,
    /windows_release_prerequisites:[\s\S]*?runs-on: windows-2022[\s\S]*?vars\.CLARK_WINDOWS_RELEASE_MODE == 'signed'/,
  );
  assert.match(
    workflow,
  ...
  assert.match(
    workflow,
    /\n  remote-code-worker:[\s\S]*?runs-on: ubuntu-22\.04[\s\S]*?environment: release/,
  );
  assert.match(
    workflow,
  ...
  assert.match(
    workflow,
    /CLARK_AUTORESEARCH_RELEASE_REF:[\s\S]*?repository: clark-labs-inc\/clark-autoresearch[\s\S]*?persist-credentials: false[\s\S]*?fetch-depth: 0/,
  );
  assert.match(
    workflow,
    /Checkout exact Clark Autoresearch dependency source[\s\S]*?--autoresearch-source "\$GITHUB_WORKSPACE\/vendor\/clark-autoresearch"/,
  );
  assert.match(workflow, /\n  build:[\s\S]*?timeout-minutes: 90/);
  assert.match(
    scientistStagingScript,
    /SCIENTIST_TARGET_DIR="\$\{CLARK_SCIENTIST_TARGET_DIR:-\$ROOT_DIR\/target\}"[\s\S]*?CARGO_TARGET_DIR="\$SCIENTIST_TARGET_DIR"/,
  );
  assert.match(
    workflow,
  ...
  assert.match(
    workflow,
    /pre_release_benchmarks:[\s\S]*?Swatinem\/rust-cache@[0-9a-f]{40}[\s\S]*?save-if: false/,
  );
  assert.match(
    workflow,
    /pre_release_benchmarks:[\s\S]*?--delegate-native-host[\s\S]*?native_host_benchmark:[\s\S]*?needs: \[release_source_prerequisites\][\s\S]*?cargo test -p clark-desktop --lib/,
  );
  assert.match(
    workflow,
    /pre_release_benchmarks:[\s\S]*?--delegate-ui-resilience[\s\S]*?ui_resilience_benchmark:[\s\S]*?needs: \[release_source_prerequisites\][\s\S]*?resilience-benchmark\.mjs[\s\S]*?--smoke/,
  );
  ...
  assert.match(
    workflow,
    /\n  build:\s*\n\s+needs: \[release_source_prerequisites, remote-code-worker\]/,
  );
  assert.match(
    workflow,
    /\n  remote-code-worker:[\s\S]*?name: Shared remote Linux coding workers[\s\S]*?needs: \[release_source_prerequisites\]/,
  );
  assert.match(
    workflow,
  ...
  assert.match(
    workflow,
    /Verify shared remote Linux coding worker artifacts[\s\S]*?clark-code-worker-x86_64-unknown-linux-musl[\s\S]*?clark-code-headless-x86_64-unknown-linux-musl[\s\S]*?RUNNER_OS[\s\S]*?== "Linux"[\s\S]*?--self-test[\s\S]*?od -An -tx1 -N4[\s\S]*?7f454c46/,
  );
  assert.match(
    ordinaryStagingScript,
    /mkdir -p "\$\(dirname "\$OUTPUT"\)"[\s\S]*?install -m 0755 "\$SOURCE" "\$OUTPUT"/,
  );
  assert.match(
    ciWorkflow,
    /tauri:[\s\S]*?CARGO_TARGET_X86_64_PC_WINDOWS_MSVC_RUSTFLAGS: "-C target-feature=\+crt-static"/,
  );
  assert.match(ciWorkflow, /os: \[macos-latest, ubuntu-latest, windows-2022\]/);
  assert.doesNotMatch(
    ciWorkflow,
    /os: \[[^\]]*windows-latest[^\]]*\]/,
  );
  assert.match(workflow, /platform: windows-2022/);
  assert.match(
    ciWorkflow,
    /cargo test -p clark-desktop --lib --no-run[\s\S]*?Copy-Item src-tauri\/windows-test\.manifest "\$\(\$testExe\.FullName\)\.manifest"[\s\S]*?check-windows-pe-imports\.ps1 -FilePath target\/debug\/clark-desktop\.exe[\s\S]*?check-windows-pe-imports\.ps1 -FilePath \$testExe\.FullName[\s\S]*?& \$testExe\.FullName[\s\S]*?--skip commands::real_backend_tests[\s\S]*?--skip project_worktree/,
  );
  assert.match(
    windowsPeCheck,
    /externalManifestPath = "\$resolvedFile\.manifest"[\s\S]*?manifestCandidates\.Add[\s\S]*?commonControlsV6[\s\S]*?missing\.Remove/,
  );
  assert.match(
    windowsTestManifest,
    /name="Microsoft\.Windows\.Common-Controls"[\s\S]*?version="6\.0\.0\.0"/,
  );
  assert.match(
    workflow,
    /--out "\$env:RUNNER_TEMP\/clark-windows-native-containment-\$env:GITHUB_RUN_ID"[\s\S]*?path: \$\{\{ runner\.temp \}\}\/clark-windows-native-containment-\$\{\{ github\.run_id \}\}/,
  );
  assert.match(
    workflow,
    /publish_independent_platforms:[\s\S]*?needs: \[build, remote-code-worker, pre_release_benchmarks, native_host_benchmark, ui_resilience_benchmark, release_source_ci\][\s\S]*?needs\.native_host_benchmark\.result == 'success'[\s\S]*?needs\.ui_resilience_benchmark\.result == 'success'/,
  );
  assert.match(
    workflow,
    /\n  publish:\s*[\s\S]*?needs:\s*\n\s+\[build, remote-code-worker, windows_packaged_journey, pre_release_benchmarks, native_host_benchmark, ui_resilience_benchmark, release_source_ci\]/,
  );
  assert.match(
    workflow,
    /publish_cli:[\s\S]*?needs: \[build, pre_release_benchmarks, native_host_benchmark, ui_resilience_benchmark, release_source_ci\][\s\S]*?needs\.native_host_benchmark\.result == 'success'[\s\S]*?needs\.ui_resilience_benchmark\.result == 'success'/,
  );
  assert.match(
    workflow,
  ...
  assert.match(
    workflow,
    /windows_release_vm_cleanup:[\s\S]*?vars\.CLARK_WINDOWS_RELEASE_MODE == 'signed'[\s\S]*?needs\.windows_packaged_journey\.result != 'skipped'[\s\S]*?--action delete-clone/,
  );
