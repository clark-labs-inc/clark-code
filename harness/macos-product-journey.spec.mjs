...
test("macOS QA Tauri config pins the dedicated persistent data store", () => {
  ...
  assert.equal(config.app.windows[0].dataStoreIdentifier, undefined);
  const host = readFileSync(path.join(repoDir, "src-tauri", "src", "lib.rs"), "utf8");
  const rustIdentifier = host.match(
    /const MACOS_QA_DATA_STORE_IDENTIFIER: \[u8; 16\] = \[([\s\S]*?)\];/,
  );
  assert.ok(rustIdentifier);
  assert.deepEqual(
    [...rustIdentifier[1].matchAll(/\b\d+\b/g)].map(([value]) => Number(value)),
    MACOS_QA_DATA_STORE_BYTES,
  );
  assert.match(host, /data_store_identifier\(MACOS_QA_DATA_STORE_IDENTIFIER\)/);
...
test("signed launcher has isolated build/launch modes and no personal label", () => {
  ...
  const profile = readFileSync(
    path.join(repoDir, "harness", "macos-qa-profile.mjs"),
    "utf8",
  );
  assert.match(profile, /tell application id \"com\.clark\.desktop\.dev\" to quit/);
});
...
test(
  "isolated profile keeps auth out of WebKit and in the native envelope",
  ...
  () => {
    ...
    try {
      ...
      const retainedAuth = {
        ...
        authOrigin: "https://example.invalid",
        clarkToken: "eyJhbGciOiJub25lIn0.eyJzdWIiOiJkZXRlcm1pbmlzdGljLXFhLWFjY291bnQifQ.signature",
        google: {
          accessToken: "",
          refreshToken: null,
          expiresAt: null,
      ...
      writeBootstrap(bootstrap, {
        ...
        marker,
        accountScope: "id:deterministic-qa-account",
        remoteHost: MACOS_QA_REMOTE_HOST,
        remoteRoot: MACOS_QA_REMOTE_ROOT,
      });
      writeNativeCredentialBootstrap(qaHome, retainedAuth);
      ...
      assert.equal(probeNativeCredentialState(qaHome, retainedAuth).status, "passed");
      assert.equal(
        existsSync(
          path.join(
            qaHome,
...
test("authenticated product journey proves a full process restart", () => {
  const source = readFileSync(
    path.join(repoDir, "harness", "macos-product-journey.mjs"),
    "utf8",
  );
  assert.match(source, /launches\.initial = launcher/);
  assert.match(source, /stopClarkDesktop\(\)[\s\S]+launches\.restart = launcher/);
  assert.match(source, /observations\.restart = observeProductReady/);
  assert.match(source, /observations\.restart_conversation = observeProductReady/);
  assert.match(source, /observations\.warm_conversation = observeProductReady/);
  assert.match(source, /"New session"/);
  assert.match(source, /CLARK_LOGS: path\.join\(qaHome, "logs"\)/);
  assert.match(source, /event\.event === "remote_worker_connected"/);
  assert.match(source, /connects\.length === 2/);
  assert.match(source, /event\.account_worker_count === 1/);
  assert.match(source, /event\.workers_shutdown === 1/);
  assert.match(source, /clickUntilRuntimeConnect[\s\S]+attempt <= 3/);
  assert.match(source, /initialSelection = clickUntilRuntimeConnect/);
  assert.match(source, /restartSelection = clickUntilRuntimeConnect/);
  assert.match(source, /await createDisposableConversation/);
  assert.match(source, /await deleteDisposableConversation/);
  const observationSource = readFileSync(
    path.join(repoDir, "harness", "macos-product-observation.mjs"),
    "utf8",
  );
  assert.match(observationSource, /reconnecting_absent/);
  assert.match(source, /schema_version: 2/);
  assert.doesNotMatch(source, /sleep\(30_000\)/);
});
