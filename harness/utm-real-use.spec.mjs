...
import { selectExpectedRemovableMedia } from "./utm-install-media.mjs";
import { shouldSendAutoinstallConfirmation } from "./utm-autonomy.mjs";
...
test("Ubuntu autoinstall confirms once even when the VM disk is preallocated", () => {
  assert.equal(shouldSendAutoinstallConfirmation(0, 60_000, 60_000), true);
  assert.equal(shouldSendAutoinstallConfirmation(0, 59_999, 60_000), false);
  assert.equal(shouldSendAutoinstallConfirmation(1, 90_000, 60_000), false);
});

test("Ubuntu installer recovery ejects only the two exact removable media", () => {
  const selected = selectExpectedRemovableMedia(
    [
      {
        device: "installer",
        removable: true,
        inserted: { file: "/host/ubuntu-desktop-arm64.iso" },
      },
      { device: "system", removable: false, inserted: { file: "/vm/system.qcow2" } },
      {
        device: "seed",
        removable: true,
        inserted: { file: "/repo/clark-cidata.iso" },
      },
      { device: "tools", removable: true, inserted: { file: "/host/guest-tools.iso" } },
    ],
    ["ubuntu-desktop-arm64.iso", "clark-cidata.iso"],
  );
  assert.deepEqual(selected, [
    { device: "installer", basename: "ubuntu-desktop-arm64.iso" },
    { device: "seed", basename: "clark-cidata.iso" },
  ]);
  assert.throws(
    () => selectExpectedRemovableMedia([], ["ubuntu-desktop-arm64.iso"]),
    /did not expose expected installer media/,
  );
});
...
test("Ubuntu product journey builds embedded ARM assets and launches without a user", () => {
  ...
  assert.match(probe, /"install",\s*"--frozen-lockfile"/);
  assert.match(probe, /frontend_root = source_root \/ "app"/);
  assert.match(probe, /os\.chown\(pathlib\.Path\(directory\) \/ name/);
  assert.match(probe, /tauri_generated_root = source_root \/ "src-tauri\/gen"/);
  assert.match(probe, /tauri_generated_root\.mkdir\(parents=True/);
...
test("Ubuntu authenticated journey seeds only Clark-owned state and erases its transfer", () => {
  const retainedAuth = {
    ...
    clarkToken: "eyJ0ZXN0Ijp0cnVlfQ.eyJzdWIiOiJxYS11c2VyIn0.signature",
    google: {
      accessToken: "",
      refreshToken: null,
      expiresAt: null,
  ...
  const probe = buildUbuntuAuthenticatedWorkspaceProbe({ retainedAuth });
  assert.match(probe, /tauri_localhost_0\.localstorage/);
  assert.match(probe, /utf-16-le/);
  assert.match(probe, /clark-desktop:local-agent:/);
  assert.match(probe, /clark-desktop:project-context:/);
  assert.match(probe, /credential_fields_absent/);
  assert.match(probe, /app_owned_encrypted_disk/);
  assert.doesNotMatch(probe, /provider_key_owner_bound/);
  assert.match(probe, /clark-code:deepseek_v4_flash_latest/);
  assert.match(probe, /loginctl", "unlock-session"/);
  assert.match(probe, /required_user_vm_actions": 0/);
  assert.doesNotMatch(probe, /customer\.example|CLARK_QA_AUTH_PASSWORD/i);
  assert.doesNotMatch(probe, new RegExp(retainedAuth.clarkToken.replaceAll(".", "\\.")));
  assert.throws(
    () => buildUbuntuAuthenticatedWorkspaceProbe({
      retainedAuth: {
        ...retainedAuth,
...
test("source staging omits tracked files deleted from the dirty worktree", () => {
  assert.deepEqual(
    excludeDeletedSourcePaths(
      ["Cargo.toml", "app/src/deleted.ts", "app/src/kept.ts"],
      ["app/src/deleted.ts"],
    ),
    ["Cargo.toml", "app/src/kept.ts"],
  );
});

test("source staging retries while a freshly booted guest agent becomes ready", () => {
  const calls = [];
  const waits = [];
  const outcomes = [
    { ok: false, stderr: "guest agent unavailable" },
    { ok: false, stderr: "guest agent unavailable" },
    { ok: true, stdout: "" },
  ];
  const result = pushSourceArchive({
    runner(command, args, options) {
      calls.push({ command, args, options });
      return outcomes.shift();
    },
    vmName: "Clark QA - Ubuntu 24.04 Desktop",
    guestArchive: "/var/tmp/source.tgz",
    archiveBytes: Buffer.from("source"),
    attempts: 5,
    retryDelayMs: 250,
  ...
  assert.deepEqual(calls[0].args, [
    "file",
    "push",
    "Clark QA - Ubuntu 24.04 Desktop",
    "/var/tmp/source.tgz",
  ]);
  assert.deepEqual(calls[0].options.input, Buffer.from("source"));
});
...
test("Windows QA settings fixture contains no auth contract", () => {
  const expression = qaLocalSettingsExpression({
    cwd: String.raw`C:\Users\home\ClarkCodeQA`,
    model: "clark-code:deepseek_v4_flash_latest",
  });
  assert.match(expression, /native_encrypted_local_account/);
  assert.match(expression, /clark-code:deepseek_v4_flash_latest/);
...
test("Clark QA login mints a short-lived account-bound session without VM input", async () => {
  ...
  assert.equal(result.retained_auth.descriptor.user.id, "qa-user-id");
  assert.equal(result.retained_auth.descriptor.user.method, "local");
  assert.equal(result.retained_auth.authOrigin, "https://www.clarkchat.com");
...
test("authenticated Windows fixture keeps credentials out of WebView storage", () => {
  const expression = qaAccountSettingsExpression({
    account: {
      id: "qa-user-id",
      name: "Autonomous VM QA",
      email: "clark-code-vm-qa@clarkslabs.com",
  ...
  assert.match(expression, /native_encrypted_account/);
  assert.match(expression, /localStorage\.clear\(\)/);
  assert.match(expression, /credential_fields_absent/);
  assert.doesNotMatch(expression, /ck_live_/);
  assert.doesNotMatch(expression, /authOrigin|"clarkToken"\s*:/);
  assert.doesNotMatch(expression, /clark-code-vm-qa@clarkslabs\.com/);
