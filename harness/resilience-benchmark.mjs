...
const simulatedTerminalTimeoutMs = 30_000;
...
function browserSeed({
  ...
  const accountScope = "id:resilience-benchmark";
  const encodedScope = encodeURIComponent(accountScope);
  const cwd = "/tmp/clark-desktop-resilience-fixture";
  localStorage.clear();
...
async function waitForTerminal(page, faults) {
  if (faults.includes("user_cancel")) {
    await page.getByText("Run stopped before finishing.").waitFor({ timeout: simulatedTerminalTimeoutMs });
  } else if (faults.includes("provider_process_loss")) {
    await page.getByText("Run interrupted.").waitFor({ timeout: simulatedTerminalTimeoutMs });
  } else {
    await page.getByText("BENCHMARK_OK").waitFor({ timeout: simulatedTerminalTimeoutMs });
  }
  await page.waitForFunction(() => Array.from(
    document.querySelectorAll('section[aria-label="Provider incident"]'),
  ).every((card) => !card.textContent?.includes("Retrying")), null, {
    timeout: simulatedTerminalTimeoutMs,
  });
}
...
async function continueAfterTerminal(page) {
  ...
  await page.getByText("BENCHMARK_OK").last().waitFor({ timeout: simulatedTerminalTimeoutMs });
}
...
async function runLiveControl() {
  ...
  const context = await browser.newContext({ viewport: VIEWPORT, reducedMotion: "reduce" });
  await context.addInitScript(({ cwd, model, storageKey }) => {
    const accountScope = "id:resilience-benchmark-live";
    const encodedScope = encodeURIComponent(accountScope);
    localStorage.clear();
    localStorage.setItem("clark.desktop.dev-account", JSON.stringify({
      user: {
  ...
  try {
    ...
    const runFailure = page.getByText(/^Run failed\./).first();
    const firstBoundary = await Promise.race([
      stopButton.waitFor({ timeout: 300_000 }).then(() => "active"),
      assistantReply.last().waitFor({ timeout: 300_000 }).then(() => "completed"),
      runFailure.waitFor({ timeout: 300_000 }).then(() => "failed"),
    ]);
    if (firstBoundary === "failed") {
      throw new Error(`live run failed: ${(await runFailure.textContent())?.trim() || "unknown error"}`);
    }
    if (firstBoundary === "active") {
    ...
    const failureScreenshot = path.join(artifactDir, "live", "deepseek-failure.png");
    await page.screenshot({ path: failureScreenshot, fullPage: true }).catch(() => {});
    const detail = [
      error instanceof Error ? error.message : String(error),
      consoleErrors.length > 0 ? `browser errors: ${consoleErrors.slice(0, 3).join(" | ")}` : "",
      devbridgeOutput.trim() ? `devbridge tail:\n${devbridgeOutput.slice(-8_000)}` : "",
...
try {
  ...
  if (includeLive) {
    try {
      report.live = await runLiveControl();
    } catch (error) {
      report.live = {
        id: "deepseek-live-control",
        kind: "live",
