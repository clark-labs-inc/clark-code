...
use self::{
    planning::{base_options, change_summary, checkout_revision, resolve_base, source_checkout},
    registry::{
...
use super::{git_output, project_branch_list, repository_root, validate_name};
...
pub async fn project_worktree_transition_plan(
    ...
    let (action, preservation, requires_confirmation, target_checkout_path) = if let Some(
        target_branch,
    ) =
        target_branch.as_deref()
    {
        let branches = project_branch_list(source.root.to_string_lossy().into_owned()).await?;
        ...
        let owned_elsewhere = match target.checkout_path.as_deref() {
            Some(owner) => {
                ...
                (owner_path != source.root).then_some(owner)
            }
            None => None,
        };
        if let Some(owner) = owned_elsewhere {
            (
                WorktreeTransitionAction::OpenOwner,
                WorktreePreservation::OwnerCheckout,
                false,
                Some(owner.to_string()),
            )
        } else if source.branch.as_deref() == Some(target_branch) {
            (
                WorktreeTransitionAction::CreateIsolated,
                if source.changes.is_dirty() {
                    WorktreePreservation::ChangesRemainInSource
                } else {
                    WorktreePreservation::Clean
                },
                source.changes.is_dirty(),
                None,
            )
        } else if source.changes.is_dirty() {
            (
                WorktreeTransitionAction::PreserveChanges,
                WorktreePreservation::ChangesRemainInSource,
                true,
                None,
            )
            ...
            (
                WorktreeTransitionAction::SwitchClean,
                WorktreePreservation::Clean,
                false,
                None,
            )
    ...
    let base_options = if action == WorktreeTransitionAction::OpenOwner {
        Vec::new()
    } else {
...
pub async fn project_managed_worktree_create(
    ...
    let (id, label) = managed_id(request.label.as_deref())?;
...
fn managed_branch_name(id: &str) -> String {
    // `id` is generated from the same ASCII-only labels accepted by
    // `validate_name`, plus a UUID suffix. Never derive a Git ref from raw UI
    // input here.
    format!("clark/{id}")
}
...
fn managed_id(label: Option<&str>) -> Result<(String, String), String> {
    let label = match label {
        Some(value) => validate_name(value)?.to_string(),
        None => "session".to_string(),
    };
    // validate_name admits ASCII only, so this byte slice cannot split a
    // character. Keep the generated path comfortably below conventional limits.
    let stem = if label.len() > 36 {
        &label[..36]
    } else {
        &label
    };
    let suffix = Uuid::new_v4().simple().to_string();
    let id = format!("{stem}-{}", &suffix[..8]);
    Ok((id, label))
}
