...
use super::{
    ...
    ManagedWorktreeBase, ManagedWorktreeBaseOption, WorktreeChangeSummary, WorktreePreservation,
...
pub(super) struct TransitionDecision {
    pub(super) action: WorktreeTransitionAction,
    pub(super) preservation: WorktreePreservation,
    pub(super) requires_confirmation: bool,
    pub(super) target_checkout_path: Option<String>,
}

/// Pure branch/worktree routing used by the native command and the exhaustive
/// journey simulation. It deliberately has no Git or filesystem access: the
/// caller supplies the already-observed checkout/owner facts, then executes a
/// separate, revalidated operation.
pub(super) fn transition_decision(
    source_root: &str,
    ...
    if let Some(owner) = target_checkout_path {
        if owner != source_root {
            return Ok(TransitionDecision {
                action: WorktreeTransitionAction::OpenOwner,
                preservation: WorktreePreservation::OwnerCheckout,
                requires_confirmation: false,
                target_checkout_path: Some(owner.to_string()),
            });
        }
    }
    ...
    if source_branch == Some(target_branch) {
        return Ok(TransitionDecision {
            action: WorktreeTransitionAction::CreateIsolated,
            preservation: if source_dirty {
                WorktreePreservation::ChangesRemainInSource
    ...
    if source_dirty {
        return Ok(TransitionDecision {
            action: WorktreeTransitionAction::PreserveChanges,
            preservation: WorktreePreservation::ChangesRemainInSource,
            requires_confirmation: true,
            target_checkout_path: None,
        });
    }
    Ok(TransitionDecision {
        action: WorktreeTransitionAction::SwitchClean,
        preservation: WorktreePreservation::Clean,
        requires_confirmation: false,
        target_checkout_path: None,
    })
}
