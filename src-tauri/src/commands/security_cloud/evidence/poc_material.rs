...
pub(super) struct ExecutedAttestationInput<'a> {
    pub root: &'a Path,
    pub record: &'a provider_local::SecurityScanRecord,
    pub platform_scan: &'a PlatformScan,
    pub candidate: &'a provider_local::security::SecurityCandidate,
    pub occurrence: &'a provider_local::ClarkSecurityOccurrenceDraft,
    pub receipt: &'a provider_local::security::SecurityPocReceipt,
    pub poc_identity: &'a ClarkSecurityScannerIdentity,
    pub poc_scanner: &'a ScannerEnrollment,
}

pub(super) async fn upload_executed_attestation(
    client: &ClarkSecurityPlatformClient,
    input: ExecutedAttestationInput<'_>,
) -> Result<Value, String> {
    let ExecutedAttestationInput {
        root,
        record,
        platform_scan,
        candidate,
