...
async fn catalog_context(
    ...
    let remote_identity = remote.as_ref().map(|remote| remote.ws_url.clone());
...
pub async fn skills_list(
    ...
    let (executor, cwd, environment_id) = catalog_context(&cwd, remote).await?;
...
pub async fn instructions_list(
    ...
    let (executor, cwd, _) = catalog_context(&cwd, remote).await?;
