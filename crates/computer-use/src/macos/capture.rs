...
const MAX_IMAGE_EDGE: f64 = 2048.0;
...
pub fn capture_window(window: &WindowInfo) -> Result<Screenshot, ComputerUseError> {
    ...
    let mut width = (window.frame.width * point_scale).round().max(1.0);
    let mut height = (window.frame.height * point_scale).round().max(1.0);
    let downscale = (MAX_IMAGE_EDGE / width.max(height)).min(1.0);
    width = (width * downscale).round().max(1.0);
    height = (height * downscale).round().max(1.0);
