...
fn parse_service_arguments(
    ...
    let usage = || {
        computer_use::ComputerUseError::HelperProtocol(
            "usage: agent-computer-use-helper --socket <absolute-path> --data-dir <absolute-path>"
                .to_string(),
        )
    };
    if arguments.len() != 4 {
...
fn main() {
    if let Err(error) = portable_main() {
        eprintln!("agent-computer-use service failed: {error}");
        std::process::exit(1);
    }
}
...
fn portable_main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    {
        return Err(
            "usage: agent-computer-use-helper --socket-name <name> --data-dir <absolute-path> --client-pid <pid>"
                .into(),
        );
    }
