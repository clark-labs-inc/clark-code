...
//!
//! Browser → server: `{id?, cmd, ...}` where cmd ∈ list_providers | connect |
//!   new_session | prompt | cancel | respond.
    ...
    match cmd.get("cmd").and_then(Value::as_str) {
        ...
        Some("connect") => {
            ...
            match provider.connect(config).await {
                Ok(()) => {
                    conn.lock().await.provider = Some(provider);
                    send(sink, json!({ "type": "ok", "id": id })).await;
                }
                Err(e) => {
                    send(
                        sink,
        ...
        Some("new_session") => {
            let options: SessionOptions =
                serde_json::from_value(cmd.get("options").cloned().unwrap_or(json!({})))
                    .unwrap_or_default();
            ...
            match provider.new_session(options).await {
                Ok(session) => {
