...
use agent_core::provider::{
    AttachmentKind, ClientResponse, EventStream, PromptInput, Provider, ProviderCapabilities,
    ProviderConfig, Session, SessionEnvironment, SessionOptions,
};
...
impl Provider for AcpProvider {
    ...
    fn capabilities(&self) -> ProviderCapabilities {
        let prompt_capabilities = self.agent_caps.get("promptCapabilities");
        ...
        if prompt_capabilities
            .and_then(|capabilities| capabilities.get("audio"))
            .and_then(Value::as_bool)
            .unwrap_or(false)
        {
            attachment_kinds.push(AttachmentKind::Audio);
        }
        if prompt_capabilities
            .and_then(|capabilities| capabilities.get("embeddedContext"))
            .and_then(Value::as_bool)
            .unwrap_or(false)
        {
            attachment_kinds.extend([
                AttachmentKind::Text,
                AttachmentKind::Pdf,
                AttachmentKind::Docx,
        ...
        ProviderCapabilities {
            ...
            fs: true,
            terminal: false,
            load_session: self.supports_load_session() || self.supports_resume_session(),
            attachment_kinds,
            modes: vec![],
    ...
    async fn new_session(&mut self, options: SessionOptions) -> Result<Session> {
        if options
            .resume
            .as_ref()
            .is_some_and(|resume| !resume.items.is_empty())
        {
            return Err(Error::Unsupported(
                "ACP cannot import a client-owned transcript into session/new; the agent must advertise loadSession or sessionCapabilities.resume"
                    .into(),
            ));
        }
        let peer = self.peer()?;
    ...
    async fn load_session(&mut self, id: SessionId) -> Result<Session> {
        ...
        let cwd = std::env::current_dir()
            ...
            .map(|p| p.display().to_string())
            .unwrap_or_else(|| ".".into());
        let method = if self.supports_load_session() {
            rpc::SESSION_LOAD
        } else if self.supports_resume_session() {
            rpc::SESSION_RESUME
        } else {
            return Err(Error::Unsupported(
                "ACP agent does not support session/load or session/resume".into(),
            ));
        };
        peer.request(
            method,
    ...
    async fn prompt(&mut self, session: &SessionId, input: PromptInput) -> Result<EventStream> {
            ...
            .find(|attachment| !supported.contains(&attachment.kind()))
        {
            return Err(Error::Unsupported(format!(
                "ACP does not support {} attachment `{}`",
        ...
        let mut prompt: Vec<Value> = input
            ...
            .collect();
        // Ingest only capability-declared inline ACP content blocks.
