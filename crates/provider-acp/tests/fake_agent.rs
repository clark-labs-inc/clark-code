...
use agent_core::domain::{AgentEvent, ContentBlock, Role, RunStatus, ToolStatus};
use agent_core::provider::{
    AttachmentKind, ClientResponse, PromptInput, Provider, ResumeItem, ResumeTranscript,
    SessionOptions,
};
...
#[tokio::test]
async fn resume_only_agent_reconnects_without_replaying_history() {
    let (client, server) = tokio::io::duplex(64 * 1024);
    let (cr, cw) = tokio::io::split(client);
    tokio::spawn(async move {
        ...
        let resume = read_msg(&mut lines).await;
        assert_eq!(resume.method.as_deref(), Some("session/resume"));
        assert_eq!(
            resume.params,
            Some(
                json!({ "sessionId": "saved-session", "cwd": std::env::current_dir().unwrap().display().to_string(), "mcpServers": [] })
            )
        );
        write_msg(
            &mut w,
            &RpcMessage::response_ok(resume.id.unwrap(), json!({})),
        )
        .await;
    });

    let mut provider = AcpProvider::new();
    provider.setup(Box::new(cr), Box::new(cw)).await.unwrap();
    let capabilities = provider.capabilities();
    assert!(capabilities.load_session);
    assert_eq!(
        capabilities.attachment_kinds,
        vec![
    ...
    let session = provider
        .load_session(agent_core::SessionId::new("saved-session"))
        .await
        .unwrap();
    assert_eq!(session.id.as_str(), "saved-session");
}

#[tokio::test]
async fn new_session_rejects_resume_history_when_agent_cannot_import_it() {
    let (client, server) = tokio::io::duplex(64 * 1024);
    let (cr, cw) = tokio::io::split(client);
    tokio::spawn(async move {
        let (r, mut w) = tokio::io::split(server);
    ...
    let error = provider
        .new_session(SessionOptions {
            resume: Some(ResumeTranscript {
                items: vec![ResumeItem::Message {
                    role: Role::User,
                    blocks: vec![ContentBlock::text("complete prior history")],
                }],
                truncated: false,
            }),
        ...
        .await
        .unwrap_err();
    assert!(error.to_string().contains("cannot import"));
}
