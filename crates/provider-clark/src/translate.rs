...
//! usage) are ignored so the run keeps streaming.
...
fn content_blocks(value: &Value) -> Vec<ContentBlock> {
    match value {
        ...
        Value::Array(values) => values.iter().flat_map(content_blocks).collect(),
        Value::Object(object) => match object.get("type").and_then(Value::as_str) {
            ...
            Some("audio") => vec![ContentBlock::Audio {
                mime_type: object
                    .get("mime_type")
                    .or_else(|| object.get("mimeType"))
                    .and_then(Value::as_str)
            ...
            Some("resource") => {
                let resource = object.get("resource").unwrap_or(value);
            ...
            _ => vec![ContentBlock::text(value.to_string())],
        },
        _ => vec![ContentBlock::text(value.to_string())],
    }
}

fn tool_result_blocks(result: Option<&Value>) -> Vec<ContentBlock> {
    let Some(result) = result else {
        return Vec::new();
    };
    if let Some(content) = result.get("content").or_else(|| result.get("output")) {
        return content_blocks(content);
    }
    if let Some(excerpt) = result.get("excerpt") {
        return content_blocks(excerpt);
    }
    content_blocks(result)
}

fn readable_thinking(data: Option<&Value>) -> Option<&str> {
    let data = data?;
    data.as_str().or_else(|| {
        ["delta", "content", "thinking", "text", "reasoning"]
            .into_iter()
...
pub fn event_to_agent(event: &Value, run: &RunId, session: &SessionId) -> Option<AgentEvent> {
    ...
    match ty {
        ...
        "assistant_thinking" => {
            let thinking = readable_thinking(data)?.to_string();
            Some(AgentEvent::MessageChunk {
                run: run.clone(),
                role: Role::Agent,
                delta: ContentBlock::thinking(thinking),
            })
        }
        ...
        "tool_result" => {
            ...
            let is_error = d.get("is_error").and_then(Value::as_bool).unwrap_or(false)
                || result
                    ...
                    .map(|ok| !ok)
                    .unwrap_or(false);
            let content = tool_result_blocks(result);
