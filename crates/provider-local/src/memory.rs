...
//!   account partition on the machine running the desktop app (always local).
//!
...
/// Account-isolated global memory directory. The opaque account binding is
/// hashed before it reaches the filesystem, and an absent binding fails closed
/// instead of falling back to an unpartitioned device-global directory.
pub fn global_memory_dir_for_scope(scope: &str) -> Option<PathBuf> {
...
/// The scope's complete index (`MEMORY.md`).
...
/// first. Accepted bodies are returned completely.
...
pub async fn scope_listing(
    ...
    if !facts.is_empty() {
        s.push_str("\nSaved notes (use `memory_recall` with action `full` for full text; re-verify CODE facts against the repo — the user's own decisions and preferences stay binding until they change them):\n");
        for f in &facts {
            let kind = f
                .header
                .kind
...
pub async fn save_memory(
    ...
    let desc = content
        ...
        .trim()
        .replace(['\n', '\r'], " ");
    // Keep the derived description on one frontmatter line; the complete fact
    // remains in the body below.
...
pub fn memory_guidance() -> &'static str {
    "You have durable memory via two deferred schemas: read-only `memory_recall`, and `memory` \
for saving or retiring facts. Plan Mode may expose `memory_recall` immediately; otherwise, \
when the needed schema is not visible, call `tool_search` with query `memory` and use the \
activated schema on the next model call. Use memory proactively, not only when asked:\n\
...
this codebase.\n\
- In a project with no saved notes yet, save the basics during early orientation when memory is \
already relevant: the stack/framework, the build and test commands, and where the entrypoint \
lives. Never activate memory after the requested work and required verification are complete \
merely to save inferred repository basics; finish the turn instead.\n\
...
- Use `memory_recall` action \"overview\" for orientation and action \"full\" only when the \
catalog exposes a material lead. Notes carry \
