...
//! [`Executor`] carried on the `ToolCtx`, instead of touching `std::fs` /
//! `tokio::process` directly. [`LocalExecutor`] is today's behavior; a future
//! `RemoteExecutor` will forward the same primitives to a remote `exec-server`.
//!
//! The tools keep their *logic* (line numbering, diffing, glob/regex matching,
//! ignore rules, output clamping) — only the raw filesystem + process primitives
//! live behind this trait, so the same tool code serves local and remote.
...
/// Metadata for a path. Only `modified` feeds the read-before-edit tracker
/// today; `len`/`is_dir` complete the stat surface the exec-server's
/// `fs/getMetadata` will serve in Phase 2.
...
/// Runs every primitive on the local machine — today's behavior, now behind the
/// [`Executor`] trait.
    ...
    async fn walk(&self, root: &Path) -> ExecResult<Vec<WalkEntry>> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            for entry in walkdir::WalkDir::new(&root)
                .follow_links(false)
                .into_iter()
                .filter_entry(|e| !crate::tools::fs::is_ignored(e.path()))
            {
