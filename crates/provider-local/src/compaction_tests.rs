...
mod tests {
    ...
    #[test]
    fn adapter_compacts_complete_readable_reasoning_findings_but_not_opaque_details() {
        let complete_reasoning = format!(
            "reasoning-begin:{}:reasoning-end",
            "0123456789abcdef".repeat(400)
        );
        let message = ca::AgentMessage::Assistant {
            content: ca::AssistantContent {
                blocks: vec![
                    ca::AssistantBlock::Reasoning(ca::TextContent {
                        text: complete_reasoning.clone(),
                    }),
                    ca::AssistantBlock::ReasoningDetails(
                        ca::ReasoningDetailsContent::from_items(&[
                            ca::ReasoningItem::Summary {
                                id: None,
                                format: ca::ReasoningFormat::OpenaiResponsesV1,
                                index: None,
                                summary: "Retry the Windows lane before release.".into(),
                            },
                            ca::ReasoningItem::Encrypted {
                                id: None,
                                format: ca::ReasoningFormat::GoogleGeminiV1,
                                index: None,
                                data: "opaque-secret-payload".into(),
                            },
                        ]),
                    ),
                ],
        ...
        assert!(output.contains(&complete_reasoning));
        assert!(output.contains("Retry the Windows lane"));
        assert!(!output.contains("opaque-secret-payload"));

        let mut config = config();
        config.compaction_prompt = "Write a concise handoff.".into();
        let config = core::with_private_reasoning_summary_guidance(&config);
        assert!(config.compaction_prompt.contains("never reproduce chain-of-thought"));
    }
    ...
    fn complete_compaction_request_includes_oldest_history_even_when_legacy_budget_is_small() {
        let transcript = vec![
            ...
            user_message("newest-history-marker ".repeat(80)),
        ];
        let views = message_views(&transcript);
        let config = CompactionConfig {
            auto_compact_token_limit: 1,
            compact_request_token_limit: 100,
