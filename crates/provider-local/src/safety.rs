...
pub fn classify_command(command: &str) -> Classification {
    ...
    // A descriptor merge contains `&`, but it is redirection rather than a
    // background-command separator. Remove it before the intentionally simple
    // segment splitter sees it.
...
fn classify_git(tokens: &[&str]) -> Classification {
    let Some(subcommand_index) = git_subcommand_index(tokens) else {
        return Classification::new(CommandRisk::Caution, "git command");
    };
    let sub = program(tokens[subcommand_index]);
    ...
    if sub == "stash" {
        ...
        return match tokens.get(subcommand_index + 1).copied().unwrap_or("push") {
...
/// Return the index of Git's subcommand after its global options. In
/// particular, `git -C <path> status` must classify `status`, not `-C`.
/// Options with values are consumed as pairs; flag-only global options are
...
fn normalize_fd_merge(command: &str) -> std::borrow::Cow<'_, str> {
    if command.contains("2>&1") {
        std::borrow::Cow::Owned(command.replace("2>&1", " "))
    } else {
...
pub(crate) fn command_requires_network(command: &str) -> bool {
    // Do not let the shell's stderr-to-stdout redirection look like an `&`
    // command separator and leave a bogus positional `2>`/`1` token behind.
    let normalized = normalize_fd_merge(command);
    split_segments(&normalized).iter().any(|segment| {
        let tokens = segment.split_whitespace().collect::<Vec<_>>();
        let Some(first) = tokens.first() else {
        ...
        let sub = if prog == "git" {
            git_subcommand_index(&tokens)
                .map(|index| program(tokens[index]))
                .unwrap_or("")
        } else {
            tokens.get(1).map(|value| program(value)).unwrap_or("")
        };
        match prog {
...
pub(crate) fn command_requires_host(command: &str) -> bool {
    let normalized = normalize_fd_merge(command);
    command_requires_network(&normalized)
        || split_segments(&normalized).iter().any(|segment| {
            let tokens = segment.split_whitespace().collect::<Vec<_>>();
            let Some(first) = tokens.first() else {
                return false;
