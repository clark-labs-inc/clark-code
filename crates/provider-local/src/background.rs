...
//! owned by the durable worker and polled through the same executor abstraction.
//! Both paths preserve complete captured output and share stdin, kill, and
//! registry limits.
...
impl BackgroundTasks {
                ...
                Ok(status) => {
                    ...
                    if status.truncated {
                        *error.lock().unwrap() = Some(
                            "remote background output exceeded the host retention boundary; complete output is unavailable"
                                .into(),
                        );
                    }
    ...
    pub async fn list(&self) -> Vec<(String, TaskStatus)> {
        let mut tasks = self
            ...
            .unwrap()
            .iter()
            .map(|(id, entry)| (entry.ordinal, id.clone()))
            .collect::<Vec<_>>();
    ...
    pub async fn clean_finished(&self) -> Vec<(String, TaskStatus)> {
        let finished = self
            .list()
            .await
            .into_iter()
            .filter(|(_, status)| status.exit_code.is_some())
            .collect::<Vec<_>>();
        let ids = finished
            .iter()
            .map(|(id, _)| id.as_str())
