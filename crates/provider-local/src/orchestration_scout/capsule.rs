...
const MAX_MODEL_VISIBLE_OUTPUT: usize = 256 * 1024;
...
fn outcome(response: CapsuleServiceResponse) -> ToolOutcome {
    match response {
        ...
        CapsuleServiceResponse::Invoked {
            ...
            if output.len() > MAX_MODEL_VISIBLE_OUTPUT {
                return ToolOutcome::ok(format!(
                    "Administrator-approved capsule `{capsule_id}` completed for `{enterprise_id}`; its oversized payload was omitted."
                ))
                .with_details(json!({
                    "registry_sha256": registry_sha256,
                    "generation": generation,
...
mod tests {
    ...
    #[test]
    fn oversized_output_returns_a_successful_receipt_without_payload() {
        let output = vec![b'x'; MAX_MODEL_VISIBLE_OUTPUT + 1];
        ...
        assert_eq!(result.details["output_omitted"], true);
        assert_eq!(
            result.details["output_bytes"],
            u64::try_from(output.len()).unwrap()
        );
        assert_eq!(result.details["isolation"]["output_sha256"], "c".repeat(64));
        assert!(result.details.get("output").is_none());
