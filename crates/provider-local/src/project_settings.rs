...
mod tests {
    ...
    fn attribution_matches_claude_defaults_customization_and_opt_out() {
        ...
        assert_eq!(defaults.pr_body_attribution(), DEFAULT_PR_BODY_ATTRIBUTION);
        ...
        // An attribution object without pr_body still defaults the PR-body note.
        assert_eq!(disabled.pr_body_attribution(), DEFAULT_PR_BODY_ATTRIBUTION);

        let pr_body_disabled: ProjectSettings = serde_json::from_value(serde_json::json!({
            "attribution": { "pr_body": "" }
        }))
        .unwrap();
        assert_eq!(pr_body_disabled.pr_body_attribution(), "");
        assert_eq!(
            pr_body_disabled.commit_attribution(),
            DEFAULT_COMMIT_ATTRIBUTION
        );
        ...
        assert_eq!(legacy_disabled.pr_body_attribution(), "");
        ...
        assert_eq!(
            object_without_commit.pr_body_attribution(),
            DEFAULT_PR_BODY_ATTRIBUTION
        );
    }
