...
pub(super) fn complete_output(bytes: &[u8]) -> String {
...
mod tests {
    ...
    #[test]
    fn accepted_output_is_returned_completely() {
        let output = format!("first failure\n{}\nfinal receipt", "x".repeat(16 * 1024));
        assert_eq!(complete_output(output.as_bytes()), output);
    }
