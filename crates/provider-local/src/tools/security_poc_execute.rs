...
mod runner;
use runner::{
    control_label, copy_inventory, digest, language_label, persist_artifacts, preview,
...
mod tests {
    ...
    /// against a directory it owns — standing in for the remote exec-server.
    ...
    #[async_trait]
    impl crate::exec::Executor for FakeRemoteExecutor {
    ...
    async fn remote_executor_routes_poc_through_the_target_service() {
        ...
        ctx.executor = Arc::new(FakeRemoteExecutor {
