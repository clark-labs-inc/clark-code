...
#[tokio::test]
async fn captured_output_is_preserved_beyond_the_old_byte_cap() {
    let dir = tempfile::tempdir().unwrap();
    let out = Bash
        .invoke(
            json!({"command": platform_command(
                "printf FIRST_FAILURE; head -c 110000 /dev/zero | tr '\\0' x; printf FINAL_SUMMARY",
                "[Console]::Out.Write('FIRST_FAILURE'); [Console]::Out.Write('x' * 110000); [Console]::Out.Write('FINAL_SUMMARY')"
            )}),
            &ctx(dir.path()),
        )
        .await;
    assert!(!out.is_error, "{}", out.content);
    assert!(out.content.contains("FIRST_FAILURE"));
    assert!(out.content.contains("FINAL_SUMMARY"));
...
fn only_opaque_mutations_across_the_host_boundary_require_effect_verification() {
    ...
    // Production incident regression: these remote-workspace inspections use
    ...
    for command in [
        ...
        assert!(
            ...
            "read-only inspection created an effect receipt: {command}"
        );
    }
}
