...
impl ActionFlavor {
    ...
    fn description(self) -> &'static str {
        match self {
            ...
            Self::SecondaryAction => "Prepare an Accessibility action explicitly advertised by an observed element and present in Clark Code's bounded allowlist.",
...
impl ToolExecutor for CommitAction {
    ...
    fn description(&self) -> &str {
        "Commit exactly one opaque prepared_action_id returned by a computer action tool. Clark Code derives the permission prompt, durable signer-bound approval eligibility, and redacted preview from trusted native preparation state. Never reconstruct or modify the action payload."
    }
