...
/// Cap on bytes returned from a single read, to keep tool results bounded.
const MAX_READ_BYTES: usize = 200_000;
/// Cap on entries listed in one directory.
...
/// A `diff <path>\n@@…` unified diff string, or None when there's no change or
/// it's too large to show inline.
fn unified_diff(path: &str, original: &str, updated: &str) -> Option<String> {
    let unified = similar::TextDiff::from_lines(original, updated)
        .unified_diff()
        .context_radius(3)
        .to_string();
    if unified.trim().is_empty() || unified.len() > MAX_DIFF_BYTES {
        None
    } else {
...
/// when the diff would be too large to show inline.
fn edit_result(
    path: &str,
    original: &str,
    updated: &str,
