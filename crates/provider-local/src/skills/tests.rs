...
async fn catalog_is_complete_and_describes_host_authority() {
    ...
    assert!(rendered.contains(description.trim()));
...
async fn discovery_preserves_deep_skills_and_more_than_the_old_count_cap() {
    ...
    write_skill(
        &root,
        "one/two/three/four/five/six/seven/eight/deep",
        "deep-skill",
        "Deep skill",
        "Deep body.",
    );
    ...
    let catalog = discover_catalog_with_home(&LocalExecutor, &project, None).await;
    assert!(catalog.resolve_name("skill-519").is_ok());
    assert!(catalog.resolve_name("deep-skill").is_ok());
    assert!(!catalog
        .diagnostics
        .iter()
        .any(|warning| warning.code == "traversal_limit"));
}
