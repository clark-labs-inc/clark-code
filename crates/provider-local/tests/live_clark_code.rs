...
use provider_local::{
    initialize_quick_chat_workspace, install_skill_pack, InstallSkillPackRequest,
    LocalAgentProvider, LocalExecutor, SkillPackAction, SkillPackScope,
};
...
fn live_matrix_accepts_product_routes_and_the_paid_qwen_eval_model() {
    ...
    assert!(is_clark_code_model("clark-code:glm52"));
    assert!(is_clark_code_model("clark-code:deepseek_v4_flash_latest"));
            ...
            *summary.event_counts.entry(event_name(&ev)).or_default() += 1;
            match ev {
                ...
                AgentEvent::ToolCall { call, .. } => {
                    ...
                    summary
                        .tool_names_by_id
                        .insert(call.id.as_str().to_string(), tool.clone());
                ...
                AgentEvent::ToolCallUpdate { id, patch, .. } => {
                    ...
                    if let Some(content) = patch.replace_content {
                        summary.tool_content.insert(id.clone(), content);
                    }
                    if !patch.append_content.is_empty() {
                        summary
                            .tool_content
                            .entry(id)
                            .or_default()
                            .extend(patch.append_content);
                    }
                }
...
async fn live_qwen_37_flash_quick_chat_paid_evaluation() {
    let Some(cfg) = live_config() else {
        return;
    };
    assert_eq!(
        cfg.model, "qwen/qwen3.7-flash",
        "the paid Quick Chat evaluation is locked to Qwen 3.7 Flash"
    );
    ...
    assert!(
        ...
        "the paid fixture must not be a Git repository"
    );
    ...
    let summary = drive_turn(
        &mut provider,
        &session.id,
        "This is a Quick Chat with no Git repository. Write the exact text quick-chat-round-trip-v1 to quick-chat-proof.txt, read the file back, then reply exactly QUICK_CHAT_EVAL_PASS.",
    )
    .await;
    println!(
        "[quick_chat_paid] provider={} model={} tools={:?} usage={:?}",
        cfg.provider, cfg.model, summary.tools, summary.usage
    );
    summary.require_done("quick_chat_paid");
    summary.require_tool("quick_chat_paid", "write_file");
    summary.require_tool("quick_chat_paid", "read_file");
    assert_eq!(summary.text.trim(), "QUICK_CHAT_EVAL_PASS");
    assert_eq!(
        std::fs::read_to_string(workspace.path().join("quick-chat-proof.txt"))
            .expect("model-created Quick Chat proof"),
        "quick-chat-round-trip-v1"
    );
    assert!(!workspace.path().join(".git").exists());
    let usage = summary.usage.expect("paid usage receipt");
    assert!(usage.input_tokens > 0 && usage.output_tokens > 0);
    assert!(
        usage.cost_usd.is_some_and(|cost| cost > 0.0),
        "paid evaluation requires positive provider-reported cost: {usage:?}"
    );
}
...
the JSON object."#,
    ...
    summary.require_done_with_tool_errors("security_adversarial");
    assert!(
        !summary
            .tools
            .iter()
...
async fn live_clark_code_feature_matrix() {
    ...
    let read_search = drive_turn(
        &mut provider,
        &session.id,
        "Use list_dir, glob, grep, and read_file. Call list_dir with path exactly `.` to list the project root, glob for **/*.rs, grep for CLARK_LIVE_GREP_SENTINEL_5142, read src/main.rs, then briefly confirm completion.",
    )
    .await;
    ...
    assert_eq!(
        read_search.last_tool_input_str("read_search", "list_dir", "path"),
        ".",
        "read_search: list_dir must use the project-relative root"
    );
