...
use agent_core::domain::{ContentBlock, RunStatus, RunUsage, ToolStatus};
...
pub(super) struct TurnSummary {
    ...
    pub(super) tool_names_by_id: BTreeMap<String, String>,
    pub(super) tool_content: BTreeMap<String, Vec<ContentBlock>>,
...
impl TurnSummary {
    ...
    /// Read-only audits may probe a stale or nonexistent path while exploring
    /// a repository. Those calls must still settle to a terminal status, but a
    /// failed read is evidence the model can recover from—not a hung run.
    pub(super) fn require_done_with_tool_errors(&self, label: &str) {
        assert!(self.finished, "{label}: run did not finish: {self:?}");
        assert_eq!(
            self.status,
            Some(RunStatus::Done),
        ...
        assert!(
            self.tool_statuses.values().all(|statuses| matches!(
                statuses.last(),
                Some(ToolStatus::Completed | ToolStatus::Failed)
            )),
            "{label}: every tool call must settle, even when a read fails: {self:?}"
        );
    }
