...
- Run Scout root and delegated model turns with the host-pinned GLM 5.2 model.
...
3. Seed a business surface manifest from authoritative entry points: source
...
4. Resolve one explicitly authorized Clark organization, system-cartography
...
5. Start `scout_ledger` with the returned census id. The host pins its
   fingerprint into the charter and rejects invented or stale ids.
6. On every local, SSH, or VM execution target, call `scout_adapter census`.
...
7. For registered GitHub, AWS, and GCP routes, use `scout_adapter fetch_page`
...
8. Append one root/coordinator-issued discovery charter. It declares the exact
