...
fn write_outcomes(path: &Path, outcomes: &[ScenarioOutcome]) -> Result<(), String> {
    ...
    for outcome in outcomes {
        writeln!(file, "{}", serde_json::to_string(outcome).unwrap())
...
async fn main() {
    ...
    let judge = Arc::new(
        JudgeClient::new(&base_url, &api_key, &args.model).expect("build memory-eval judge"),
    );
    ...
    write_outcomes(&args.out, &outcomes).unwrap_or_else(|error| panic!("{error}"));
...
mod tests {
    ...
    #[test]
    fn outcome_writer_creates_missing_parent_directories() {
        let root = tempfile::tempdir().unwrap();
        let path = root.path().join("nested/results.jsonl");
