...
impl JudgeClient {
    pub fn new(base_url: &str, api_key: &str, model: &str) -> Result<Self, String> {
        let session_id = format!("memory-eval-judge-{}", uuid::Uuid::new_v4());
    ...
    pub async fn judge(&self, rubric: &str, subject: &str) -> (bool, String) {
        let system = "You are a strict evaluator. Apply the rubric to the material. Reply with ONLY a JSON object: {\"pass\": true|false, \"why\": \"<one sentence>\"}";
        let user = format!("RUBRIC:\n{rubric}\n\nMATERIAL:\n{subject}");
        for attempt in 0..2 {
            let content = self
                .llm
                .complete_text(Some(system), &user, &CancellationToken::new())
                .await;
