...
mod tests {
    ...
    #[test]
    fn receipt_writer_creates_missing_parent_directories() {
        let root = tempfile::tempdir().unwrap();
        let path = root.path().join("nested/results.json");
        write_receipt(&path, &json!({"status": "failed"})).unwrap();
        assert_eq!(
...
async fn main() {
    ...
    if let Ok(path) = std::env::var("GOAL_EVAL_RESULTS_JSON") {
        ...
        write_receipt(Path::new(&path), &receipt).unwrap_or_else(|error| panic!("{error}"));
