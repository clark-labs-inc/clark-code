...
import { UPDATE_DRAIN_TIMEOUT_MS } from "./sessionStore.runtime";
...
describe("update coordinator", () => {
  ...
  it("releases the waiting state when a native run never drains", async () => {
    vi.useFakeTimers();
    try {
      updater.beginUpdateDrain.mockResolvedValue(1);
      useSessionStore.setState({ update: { version: "0.1.65" } });

      const applying = useSessionStore.getState().applyUpdate();
      await vi.advanceTimersByTimeAsync(UPDATE_DRAIN_TIMEOUT_MS);
      await applying;
      ...
      expect(useSessionStore.getState().error).toContain("native update drain appears stuck");
    } finally {
      vi.useRealTimers();
    }
  });
