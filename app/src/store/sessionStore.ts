...
export const useSessionStore = create<import("./sessionStore.runtime").SessionState>((set, get) => ({
  ...
  /** Bumped whenever the persisted SSH host list changes so UI that reads
   *  `startBlockedReason()` (which re-reads localStorage) re-evaluates. */
  sshHostsRevision: 0,
  activeRemote: null,
  ...
  collaborationModes: loadCollaborationModes(bootAccountScope),
