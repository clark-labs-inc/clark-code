...
/** Build `@`-mention suggestions from a file listing. Directory candidates are
 * derived from each file's ancestors, then ranked ahead of file matches so a
 * folder query points the agent at the folder instead of an arbitrary file in
 * it. */
export function fuzzyFilterProjectPaths(
  paths: string[],
  query: string,
  limit = 8,
  ...
  const directoryMatches = fuzzyFilterFiles(
    directories,
    query,
    directories.length,
  ).map((path): ProjectPathSuggestion => ({ kind: "directory", path }));
  const fileMatches = fuzzyFilterFiles(paths, query, paths.length)
    .map((path): ProjectPathSuggestion => ({ kind: "file", path }));

  return [...directoryMatches, ...fileMatches].slice(0, limit);
}
