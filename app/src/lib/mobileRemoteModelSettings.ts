...
export function mobileRemoteModelSettings(
  ...
  const rawEffort = hasEffort ? payload.reasoning_effort : config.defaultReasoningEffort;
  if (typeof rawEffort !== "string") {
    throw new Error("The selected Clark Code reasoning effort is invalid.");
  }
  const normalized = normalizeReasoningEffort(model, rawEffort);
  if (normalized !== rawEffort) {
    throw new Error("That reasoning effort is not available for the selected Clark Code model.");
  }
  return { model, reasoningEffort: normalized };
}
