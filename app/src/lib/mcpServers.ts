...
export interface McpServerConfig {
  /** Opaque native encrypted-store lookup key; never a credential value. */
...
export function saveMcpServers(list: McpServer[], scope?: string | null): void {
  try {
    localStorage.setItem(accountScopedKey(KEY, scope), JSON.stringify(redactMcpSecrets(list)));
  } catch {
    /* quota — best effort */
  }
}
...
export function enabledMcpConfigs(list: McpServer[]): McpServerConfig[] {
  return list
    ...
    .map((s) => ({
      ...
      env: Object.fromEntries(Object.keys(s.env).map((name) => [name, ""])),
    }));
}

/** Keep only environment names in renderer persistence and provider config. */
export function redactMcpSecrets(list: McpServer[]): McpServer[] {
