  ...
  return (
    <div ref={ref} className="relative shrink-0">
      <button
        ...
        aria-expanded={open}
        aria-label={agentCount > 0 ? `${agentCount} other agents active in this checkout` : "No other agents detected in this checkout"}
      ...
      >
        ...
        <span>{agentCount > 0 ? `${agentCount} agent${agentCount === 1 ? "" : "s"}` : "Solo"}</span>
        <Circle
          className={cn(
      ...
      {open && (
        <div
          role="dialog"
          aria-label="Parallel work details"
          className="popover-surface absolute bottom-full right-0 z-50 mb-2 w-[min(22rem,calc(100vw-3rem))] overflow-hidden rounded-2xl bg-bg-elevated shadow-lifted ring-1 ring-border-subtle"
        >
          <div className="border-b border-border-subtle px-3.5 py-3">
            <div className="flex items-center gap-2">
              ...
              <div className="min-w-0">
                <p className="text-sm font-semibold text-ink">Parallel work</p>
                <p className="truncate text-xs text-ink-muted">{branch} · this checkout</p>
              </div>
            </div>
          </div>

          <div className="grid grid-cols-3 gap-px bg-border-subtle">
            <Stat value={agentCount} label="agents" active={agentCount > 0} />
