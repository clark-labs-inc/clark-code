 ...
 *  Rendered whenever a pending request is displayed: whether the active
 *  approval policy would auto-grant it is decided by the caller (Conversation),
 *  which keeps this card mounted long enough for AnimatePresence to animate its
 *  exit instead of tearing the content out mid-frame. */
export function PermissionGate({ req }: { req: PermissionRequest }) {
  ...
  const onPick = (option: PermissionOption) => {
    if (picked) return;
    if (option.kind === "allow_always" && isShell && req.detail) {
      allowCommand(project, req.detail, accountScope);
    }
    setPicked(option.id);
    resolve(option.id)
      .then(() => {
      ...
      <div className="flex flex-wrap gap-2">
        {req.options.map((option) => (
          <button key={option.id} type="button" onClick={() => onPick(option)} disabled={picked !== null} className={cn("relative rounded-lg px-3 py-1.5 text-sm font-medium transition disabled:opacity-50", OPTION_STYLE[option.kind])}>
            ...
            {picked === option.id && <span className="absolute inset-0 grid place-items-center"><Loader2 className="approval-spinner size-3.5" /></span>}
          </button>
        ))}
      </div>
    </div>
