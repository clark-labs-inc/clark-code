...
export function SecurityButton() {
  ...
  const apiKey = useSessionStore((state) => state.localSettings.apiKey);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      setRecords(await activeBridge.listSecurityScans(cwd));
    } catch (cause) {
  ...
  const connectRepository = useCallback(async (
    ...
    if (!creds || !cwd) return;
    selectSecurityOrganization(fingerprint, organization.id);
    ...
    try {
      ...
      if (!apiKey.trim()) {
        setCloud({
          ...
          registration,
          syncError:
            "Local evidence sync will retry automatically when Clark Code finishes provisioning this account.",
        });
        return;
      }
      try {
  ...
  const loadCloud = useCallback(async () => {
    ...
    try {
      ...
      const selected = selectedSecurityOrganization(repository.fingerprint);
