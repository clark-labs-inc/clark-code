...
function currentProjects(): CodeRemoteProjectRegistration[] {
  ...
  const accountScope = codeKeyAccountBinding(useSessionStore.getState().auth);
  for (const host of loadSshHosts(accountScope).filter(hostReady)) {
    projects.push({
...
async function applyProject(command: CodeRemoteCommand): Promise<void> {
  ...
  const host = loadSshHosts(codeKeyAccountBinding(state.auth)).find((item) => sshProjectId(item.id, item.remoteRoot.trim()) === project.id);
  if (!host) {
    remoteFailure(
      "project_unavailable",
...
export async function steerRun(command: CodeRemoteCommand): Promise<void> {
  ...
  if (!state.bridge?.steer) {
    remoteFailure("desktop_unavailable", "This desktop session cannot accept steering.", true);
  }
...
export async function compactConversation(command: CodeRemoteCommand): Promise<void> {
  ...
  if (!state.bridge?.compact) {
    remoteFailure("desktop_unavailable", "This desktop session cannot compact context.", true);
  }
