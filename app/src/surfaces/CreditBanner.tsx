...
export function creditBannerMessage(
  ...
  const workspaceCovered = billingState.ownerKind === "organization";
  const percent = billingState.usage.percentUsed;
  if (state === "out") {
...
export function CreditBanner() {
  ...
  const state = projectClarkCodeBilling(billing).usage.state;
