...
import { billingFailureNeedsAction, upgradePromptCopy } from "./UpgradePrompt";
...
describe("Clark Code billing coverage copy", () => {
  ...
  it("uses the effective workspace balance instead of the empty personal wallet", () => {
    expect(projectClarkCodeBilling(workspaceBilling(7_577)).usage.state).toBe("ok");
  });

  it("retires a historical 402 prompt once workspace billing recovers", () => {
    expect(billingFailureNeedsAction(null)).toBe(true);
    expect(billingFailureNeedsAction(workspaceBilling(0))).toBe(true);
    expect(billingFailureNeedsAction(workspaceBilling(40_000))).toBe(false);
  });

  it("treats only DeepSeek Flash as included", () => {
    expect(isIncludedCodingModel("clark-code:free")).toBe(true);
    expect(isIncludedCodingModel("clark-code:deepseek_v4_flash_latest")).toBe(false);
