...
usage() {
  ...
  echo "          [--delegate-native-host] [--delegate-ui-resilience]"
  ...
  echo "--delegate-native-host records native host contracts as a separate CI gate."
  echo "--delegate-ui-resilience records the browser sample as a separate CI gate."
...
superpowers=""
run_live=1
delegate_native_host=0
delegate_ui_resilience=0
run_utm_preflight=0
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --offline)
      ...
      shift
      ;;
    --delegate-native-host)
      delegate_native_host=1
      shift
      ;;
    --delegate-ui-resilience)
      delegate_ui_resilience=1
      shift
      ;;
    --utm-preflight)
      run_utm_preflight=1
...
preflight_args=(
  node "$repo_root/harness/release-environment-preflight.mjs"
  --out "$output/environment-preflight"
)
if [[ "$run_live" == "1" ]]; then
  preflight_args+=(--require clark)
fi
if [[ "$run_utm_preflight" == "1" ]]; then
  preflight_args+=(--require utm)
fi
if ! "${preflight_args[@]}"; then
  echo "Release environment preflight failed; refusing to start the benchmark." >&2
  exit 2
fi
...
core_status="failed"
if [[ "$delegate_native_host" == "1" ]]; then
  native_status="delegated"
else
...
if [[ "$delegate_ui_resilience" == "1" ]]; then
  ui_status="delegated"
else
  ui_status="failed"
fi
...
if [[ "$delegate_native_host" == "0" ]]; then
  ...
  echo
  echo "Native host contracts are delegated to their isolated release workflow gate."
fi
...
if [[ "$delegate_ui_resilience" == "0" ]]; then
  ...
  echo
  echo "UI resilience is delegated to its isolated release workflow gate."
fi
...
if [[ "$delegate_native_host" == "0" && "$native_status" != "passed" ]]; then
  contract_families_passed=0
fi
if [[ "$delegate_ui_resilience" == "0" && "$ui_status" != "passed" ]]; then
  contract_families_passed=0
fi
